...
exports.lookupAddress = exports.resolveName = void 0;
const Client_js_1 = require("../lib/client/Client.js");
...
const resolveName = async (name) => {
    ...
    const url = `${global_constants_js_1.BASE_URL}/api/v1/cdc-developer-platform/cronosid/resolve/${name}?chainId=${chainId}`;
    try {
...
const lookupAddress = async (address) => {
    const chainId = Client_js_1.Client.getChainId();
    const url = `${global_constants_js_1.BASE_URL}/api/v1/cdc-developer-platform/cronosid/lookup/${address}?chainId=${chainId}`;
    try {
        const response = await fetch(url, {
            headers: {
                'Content-Type': 'application/json',
